...
export function HeroCarousel() {
  ...
  return (
    <div>
      <section className="relative h-[78vh] min-h-[520px] w-full overflow-hidden">
        {slides.map((slide, index) => (
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-primary-deep/90 via-primary-deep/60 to-transparent" />
          </div>
        ))}

        <div className="container mx-auto relative flex h-full flex-col justify-center px-4 sm:px-6">
          <div key={active} className="animate-fade-up max-w-xl text-primary-foreground">
            <span className="inline-flex items-center gap-2 rounded-full bg-accent px-4 py-1.5 text-xs font-bold uppercase tracking-[0.2em] text-accent-foreground">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 rounded-full bg-primary-foreground px-6 py-3 text-sm font-bold text-primary-deep transition-transform hover:scale-105"
              >
                Shop the collection <ArrowRight className="h-4 w-4" />
              </Link>
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/50 px-6 py-3 text-sm font-bold text-primary-foreground transition-colors hover:bg-primary-foreground/10"
              >
                Women&apos;s picks
              </Link>
            </div>
            ...
            {slides.map((slide, index) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  index === active ? "w-10 bg-accent" : "w-4 bg-primary-foreground/50"
                }`}
              />
            ))}
          </div>
      ...
      <div className="brand-gradient overflow-hidden py-3 text-primary-foreground">
