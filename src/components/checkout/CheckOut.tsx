...
const schema = z.object({
  ...
  address: z.string().trim().min(10, "Please enter a complete shipping address").max(300),
});
...
export default function Checkout() {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    dispatch(clearCart());
    setOrder(created);
    toast.success("Order placed successfully", { description: `Order ID ${created.id}` });
  };
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center sm:px-6">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Thank you {order.name}. Your order <span className="font-bold">{order.id}</span> has been
          placed successfully. We will call {order.phone} to confirm delivery.
        </p>
        ...
        <div className="mt-8 rounded-xl border border-border bg-card p-6 text-left">
          ...
          <div className="mt-4 flex items-end justify-between border-t border-border pt-4">
            <span className="text-sm tracking-[0.18em] text-muted-foreground">GRAND TOTAL</span>
            <span className="text-3xl font-bold text-primary">{formatPrice(order.total)}</span>
          </div>
          <p className="mt-4 text-xs text-muted-foreground">Shipping to: {order.address}</p>
        </div>
  ...
  return (
    <div className="container mx-auto px-4 py-10 sm:px-6 sm:py-14">
      <p className="text-xs font-bold uppercase tracking-[0.24em] text-primary">Checkout</p>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <form onSubmit={submit} className="rounded-xl border border-border bg-card p-6">
          ...
          <Button type="submit" size="lg" className="mt-7 w-full shadow-brand">
          ...
          <ul className="mt-4 space-y-3 text-sm">
            {cart.map((line) => (
              <li key={`${line.productId}-${line.size}`} className="flex justify-between gap-3">
                ...
                <span className="shrink-0 font-semibold">{formatPrice(line.price * line.quantity)}</span>
              </li>
            ))}
          </ul>
